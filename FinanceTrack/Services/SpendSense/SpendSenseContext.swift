...
/// Everything a `SpendSenseEngine` is allowed to read, built once per generation pass by the
...
/// The Spend Sense foundation is local and synchronous — there is no genuine concurrency need
...
struct SpendSenseContext {
    ...
    /// caller, never read from `Date()` inside a `SpendSenseEngine`, so every engine's output is
